    ...
    def getGT(self):
        ...
        if os.path.isfile(self.cachePath) and os.path.exists(self.cachePath):
            ...
            for annotation in annotations:
                ...
                if ("%d" % annotation["image_id"]) not in whole_gt[
                    "%d" % annotation["category_id"]
                ]:
                    whole_gt["%d" % annotation["category_id"]][
                        "%d" % annotation["image_id"]
                    ] = []
                    whole_gt["%d" % annotation["category_id"]][
                        "%d" % annotation["image_id"]
                    ].append(
    ...
    def getPR(self):
        ...
        for idx, det in enumerate(tqdm(self.dets)):
            ...
            if gtBBOX.size() > 0:
                ...
                # union
                uni = (
                    (detBBOX[2] - detBBOX[0] + 1.0) * (detBBOX[3] - detBBOX[1] + 1.0)
                    + (gtBBOX[:, 2] - gtBBOX[:, 0] + 1.0)
                    * (gtBBOX[:, 3] - gtBBOX[:, 1] + 1.0)
                    - intersection
                )
                ...
                if ioumax > self.iouThresh:
                    if not self.gt[imgID][jmax]["difficult"]:
                        ...
                        if not self.gt[imgID][jmax]["detected"]:
                            ...
                            self.gt[imgID][jmax][
                                "detected"
                            ] = 1  # mark the gt bbox as detected
                        else:
                            fp[idx] = 1.0
                else:
                    fp[idx] = 1.0
