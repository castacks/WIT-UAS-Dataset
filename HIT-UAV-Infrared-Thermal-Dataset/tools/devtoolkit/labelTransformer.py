...
class LabelTransformer(object):
    ...
    def getJSONPath(self):
        ...
        for annoFile in annoFiles:
            self.jsonPaths[annoFile] = (
                Path(self.jsonFolder).joinpath(annoFile).with_suffix(".json")
            )
            assert (
                Path(self.jsonPaths[annoFile]).exists() == True
            ), "[!] %s is lost" % str(self.jsonPaths[annoFile])
    ...
    def toVOC(self):
        ...
        for key, value in self.jsonPaths.items():
            ...
            for idx in trange(totalImgs):
                ...
                if self.copyIMG:
                    shutil.copy(
                        Path(self.imgFolder).joinpath(key, img_name),
                        Path(vocImgFolder).joinpath(img_name),
                    )
                ...
                for i in range(totalAnnotations):
                    ...
                    if annotations[i]["image_id"] == img_idx:
                        ...
                        obj_name.text = jsonFile["categories"][
                            int(annotations[i]["category_id"])
                        ]["name"]
    ...
    def toCOCO(self):
        ...
        for key, value in self.jsonPaths.items():
            ...
            for imgAnno in jsonFile["images"]:
                ...
                if self.copyIMG:
                    shutil.copy(
                        Path(self.imgFolder).joinpath(key, imgAnno["filename"]),
